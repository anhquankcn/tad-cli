...
describe('mapStopReason / mapUsage', () => {
  ...
  it('treats the Bedrock tool-pairing 400 as transient, but not the id mismatch', () => {
    ...
    expect(mapStopReason(assistant({
      stopReason: 'error',
      errorMessage: '400:{"message":"service error message: The number of toolResult blocks at messages.6.content exceeds the number of toolUse blocks"}',
    }))).toMatchObject({ kind: 'error', failure: { code: 'SERVER' } })

    // A mid-session model switch leaves tool ids minted by the other provider
    // in history. That rejection IS deterministic — replaying cannot fix it, so
    // it must stay INVALID_REQUEST and not consume the retry budget.
    expect(mapStopReason(assistant({
      stopReason: 'error',
      errorMessage: '400:{"message":"messages.36.content.1: unexpected tool_use_id found in tool_result blocks: toolu_bdrk_01ABC"}',
    }))).toMatchObject({ kind: 'error', failure: { code: 'INVALID_REQUEST' } })

    // The narrow rule must not swallow ordinary 400s.
    expect(mapStopReason(assistant({
      stopReason: 'error',
      errorMessage: 'HTTP 400: unsupported parameter "top_k"',
    }))).toMatchObject({ kind: 'error', failure: { code: 'INVALID_REQUEST' } })
  })

  it('tells the operator how to recover from the tool-id mismatch', () => {
    const failure = mapStopReason(assistant({
      stopReason: 'error',
      errorMessage: '400:{"message":"messages.52.content.1: unexpected tool_use_id found in tool_result blocks: toolu_bdrk_01ABC"}',
    }))

    // The provider text stays verbatim — the advice is added, never a replacement.
    expect(failure).toMatchObject({
      kind: 'error',
      failure: {
        code: 'INVALID_REQUEST',
        message: expect.stringContaining('unexpected tool_use_id') as string,
      },
    })
    expect(failure).toMatchObject({
      failure: { message: expect.stringContaining('đổi model giữa phiên') as string },
    })
    expect(failure).toMatchObject({
      failure: { message: expect.stringContaining('phiên mới') as string },
    })

    // Other failures must not collect advice they have no use for.
    expect(mapStopReason(assistant({ stopReason: 'error', errorMessage: 'HTTP 500: upstream unavailable' })))
      .toMatchObject({ kind: 'error', failure: { message: 'HTTP 500: upstream unavailable', code: 'SERVER' } })
  })
