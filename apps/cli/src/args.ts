/**
 * Commander adapter for the `tad` command line.
 *
 ...
 * `tad --profile tui --resume abc` boots the tui profile with `--resume abc`,
 * and `tad --profile web -h` prints the web app's help, not this one's.
 *
...
/** The resolved `tad` invocation. Help, version, and errors exit inside {@link parseDshArgs}. */
...
const HELP_EXAMPLES = `
Examples:
  tad --profile web                          boot the web profile (same as: tad web)
  tad --profile headless "run the tests"     answer one task, print the result, and exit
  tad --profile tui --patch ./extra.yml      boot a custom profile with one extra overlay
  tad --profile tui --resume <session>       arguments after the launcher flags reach the app
  tad --profile web --help                   the web app's own flags and help
  tad plugin --profile tui add <package>     install a plugin into the tui profile
`
...
export function parseDshArgs(argv: readonly string[], version: string): DshInvocation {
  ...
  program
    ...
    // its -h. `tad -h` with no profile still prints this help, below.
    .helpOption(false)
    .allowUnknownOption()
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the booted profile\'s app (see: tad --profile <name> --help)')
    ...
    .action((args: string[], options: BootOptions & { profile?: string }) => {
      ...
      // `tad -h` (no profile to hand it to) must print.
  ...
  web
    ...
    .passThroughOptions()
    .enablePositionalOptions()
    .argument('[args...]', 'arguments for the web app (see: tad web --help)')
    .option('--patch <path>', 'extra patch-list overlay applied after the profile layer (repeatable)', collect)
  ...
  // subcommand name before the root's `[args...]`, so `tad --profile headless
